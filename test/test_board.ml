...
let test_place () =
  ...
  let init = Board.empty in
  let board = Board.place player_id row col init in
...
let test_player_on_rows () =
  let player_ids = List.init 3 Fun.id |> List.map Player_id.of_int in
  let rows = [ 0; 1; 2 ] |> List.map Row.of_int_exn in
  let pairs = List.combine player_ids rows in
  let col = Column.of_int_exn 0 in
  let board =
    List.fold_left (fun acc (i, r) -> Board.place i r col acc) Board.empty pairs
  in
  List.iter
    (fun (i, r) ->
